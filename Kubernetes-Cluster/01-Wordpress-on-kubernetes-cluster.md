01-Wordpress-on-kubernetes-cluster


## Here in this Notes, what will do is?:
- firstly we will deploy `wordpress` in `docker-container` by using `docker-compose`
- then we will deploy same `wordpress` in pods byusing `kubernetes-cluster`
...
### create `yaml` script for deploy `wordpress` & `mysql` inside pods:
```
vim wp-mysql.yml
...
#### Noted: here we are not getting `wordpress` dashboard via `public-ip` of instance.
